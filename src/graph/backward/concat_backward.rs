...
impl<T> Backward<T> for ConcatBackward<T>
where
    T: Clone + DTComp + Debug + 'static + Add<Output = T>,
{
    ...
    fn calculate_gradient_for_next_node(
        ...
        if let Some(edge) = edge {
            ...
            let (start, end) = self.ranges[input_nr];

            let grad_slice = upstream_gradient
                .get_raw_data()
                .slice_axis(Axis(self.dim.index()), Slice::from(start..end))
                .to_owned();

            let tensor = Tensor::from_raw_array(grad_slice, false);
    ...
    T: Clone + DTComp + Debug,
{
    ...
    pub fn save_ranges(&mut self, range: (usize, usize)) {
