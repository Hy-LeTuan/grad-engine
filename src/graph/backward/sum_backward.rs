...
impl<T> Backward<T> for SumBackward<T>
where
    T: Clone + DTComp + Debug + 'static + Add<Output = T>,
{
    ...
    fn calculate_gradient_for_next_node(
        ...
        if let Some(_) = edge {
            ...
            let result_tensor = Tensor::from_raw_array(broadcasted_data, false);
    ...
    T: Clone + DTComp + Debug,
{
    pub fn new(id: usize, edge_list: Vec<Edge<T>>, origin: &Rc<RefCell<TensorImpl<T>>>) -> Self {
        let node = SumBackward {
            name: BackwardType::SumBackward,
            input_refs: vec![],
            id,
            edge_list,
    ...
    fn sum_backward_operation() {
        ...
        if z.does_require_grad() {
            assert_eq!(
                ...
                String::from("SumBackward"),
                "SumBackward does not exist on tensor sum operation"
            );
        }
    }
}
