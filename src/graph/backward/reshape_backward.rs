...
impl<T> Backward<T> for ReshapeBackward<T>
where
    ...
    fn calculate_gradient_for_next_node(
        ...
        if let Some(_) = edge {
            if let Some(axes) = &self.axes_option {
                let result_tensor = compute_reshape(upstream_gradient.deref(), axes.to_vec());

                return Rc::new(result_tensor);
            } else {
                panic!("Error: Original shape is needed to compute gradient from reshape node");
            }
        } else {
            panic!(
